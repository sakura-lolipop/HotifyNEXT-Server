...
- **鉴权**：**云函数中转**——服务账号 RSA 私钥 / PS256 JWT 锁云函数（hotifypushkit.netlify.app），Server 只 POST 云函数（带 token + notification），**不直签 JWT**（CP4 纠正：旧「JWT 直签」是过时幻觉，私钥从不在 Server；详见 docs/pushkit-transport.md）。
- **推送**：云函数内 `POST https://push-api.cloud.huawei.com/v3/{project_id}/messages:send`，header `push-type:0`，成功码 `80000000`。
...
- **region**：仅中国境内（region=CN）。`testMessage=false`（有 SUBSCRIPTION 权益，服务/通讯类无频控，不自用绕频控）。
...
- Push Kit JWT：**云函数签**（PS256，私钥锁云函数 hotifypushkit.netlify.app；Server 不持私钥，仅 `net/http` POST 云函数中转，CP4 实装）。
