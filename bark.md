...
## 2. HotifyNEXT 字段映射表（bark → PushSpec / Message）

对照 `internal/model/model.go:39-63`（Message / PushSpec）+ `docs/NEXT-Server.md` §6/§13b。
...
| `title` | 通知标题 | `PushSpec.Title` / `Message.Title` | 直映 |
| `body` | 正文 | `PushSpec.Body` / `Message.Body` | 直映；路径式多段 body 合并（`route_push.go` 无此逻辑但 bark 客户端惯例——现有 `bark.go:53` 已合并 `segs[2:]`） |
...
| `badge` | 角标 | ⚠️ 待验 | 鸿蒙角标走 App setBadgeNumber，PushSpec 暂无字段；CP3 可丢或留 ext |
| `call`（=`1`） | 来电持续响铃 | `PushSpec.Category = "call"` | §6/§13b 最干净的类型映射；原生 category 一等字段，bark 命中 call 直接归一 |
| `level`（`critical/active/timeSensitive/passive`） | iOS 中断级 | `PushSpec.Category` 近似 + profile alert 强度 | §6：critical 在鸿蒙**无真穿透**，伪 critical = 系统关键词置顶；映射到 category 是近似（非 1:1） |
| `group` | 通知分组 | `PushSpec.Category`（挪用）/ 元数据 | §6：用户在 SmsForwarder 按 `group="verify"` 配 → category=verify；也进历史元数据 App 分组显示 |
| `url` | 点击跳转 | PushKit `clickAction.data`（§附录 A 必填） | §6 bark 功能映射；clickAction 是 PushKit 必填字段，bark `url` 正好填进去 |
| `icon` | icon URL | **鸿蒙 N/A** | §6 bark 不能做：iOS iMessage 专属，鸿蒙无；§15 profile API 是 App 内补全（锁屏做不到） |
| `image`（bark v3 新增，⚠️ 待验） | 通知图 | `PushSpec.MediaIDs`（单图归一成长度 1）+ server mini ≤192KB | §6/§4b：PushKit `notification.image`；API_V2.md 未列但 bark App 已支持，需查 bark App 源码二次确认字段名 |
...
### ✅ 能映射进 PushSpec（bark 兼容皮能做到的子集）
- `title` / `body` / `device_key`（→ TargetUUID）—— 直映，现有 `bark.go` 已做。
- `call` → `category = "call"`（§13b 业务 category 来源第 1 条）。
- `level` / `group` → `category` 近似映射（§6：group 挪用当 category 标签；level→alert 强度）。
- `url` → PushKit `clickAction.data`（§附录 A clickAction 必填，bark url 正好）。
- `image` → `MediaIDs`（单图归一长度 1，mini ≤192KB，§4b）—— ⚠️ 待验字段名。
...
- **多段 body 合并**：路径式 `/{key}/title/body/with/slashes` 把 `segs[2:]` 用 `/` join（现有 `bark.go:53` 已做，bark-server 源码无此逻辑但客户端惯例——保留）。
...
- **空消息兜底**（`route_push.go:254-257`）：title/body/subtitle 全空时 body 填占位（防丢）。
...
- `level` 非空 → 映射 category 近似（active/timeSensitive/critical/passive；critical 标伪穿透）—— 优先级低于显式 call/group。
