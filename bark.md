...
> 用途：CP3 `bark /{device_key}` 兼容端点实装的权威依据，对应 `internal/server/bark.go`（CP3a 搬进 server 包，砍独立 bark 包；CP3c 九缺口已补齐）。
...
对照 `internal/server/bark.go`（CP3c 重写前 CP1-temp 仅 title/body、category 硬编 default；CP3c 已全补齐，commit 6652fdd）：
