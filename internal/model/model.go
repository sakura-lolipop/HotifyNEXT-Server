...
type Message struct {
	...
	URL        string            `json:"url,omitempty"`         // 点击跳转 URL（bark url / 原生 url → PushKit clickAction.data）。TD-12 CP4 已 sanitize：bark.go:292 + push.go:150 + harmony.go:95 三处收 URL 前 util.SanitizeActionURL（javascript:/file: 拒）
