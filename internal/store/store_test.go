...
// TestBBolt_MessagesLatest 最新 N 升序（TD-19）：存 60 条 → MessagesLatest(50) 返最新 50（旧→新），最老 10 排除。
// 修 TD-19（MessagesSince(0,50) 返最老 50 的 CP1 临时 bug）。
func TestBBolt_MessagesLatest(t *testing.T) {
	st := newTestBBolt(t)
	for i := 0; i < 60; i++ {
		st.SaveMessage(model.Message{Category: "default", Body: fmt.Sprintf("m%d", i)})
	}
	got, err := st.MessagesLatest(50)
	if err != nil {
		t.Fatal(err)
	}
	if len(got) != 50 {
		t.Fatalf("MessagesLatest(50): got %d, want 50", len(got))
	}
	// 最新 50 = m10..m59（最老 m0..m9 排除）
	if got[0].Body != "m10" {
		t.Errorf("first（最旧 of 最新50）: got %q, want m10", got[0].Body)
	}
	if got[49].Body != "m59" {
		t.Errorf("last（最新）: got %q, want m59", got[49].Body)
	}
	// 升序（旧→新）：HLC 严格递增
	for i := 1; i < len(got); i++ {
		if got[i].HLC <= got[i-1].HLC {
			t.Errorf("not ascending at %d: hlc=%d <= prev=%d", i, got[i].HLC, got[i-1].HLC)
		}
	}
}

// TestBBolt_MessagesLatest_FewerThanLimit 存 <limit 条 → 返全部（不凑数）。
func TestBBolt_MessagesLatest_FewerThanLimit(t *testing.T) {
	st := newTestBBolt(t)
	for i := 0; i < 5; i++ {
		st.SaveMessage(model.Message{Category: "default", Body: fmt.Sprintf("m%d", i)})
	}
	...
	if len(got) != 5 {
		t.Errorf("5 msgs, MessagesLatest(50): got %d, want 5", len(got))
	}
}

// TestMemory_MessagesLatest Memory 语义同 BBolt（TD-19）。
func TestMemory_MessagesLatest(t *testing.T) {
	st := NewMemory()
	for i := 0; i < 60; i++ {
		st.SaveMessage(model.Message{Category: "default", Body: fmt.Sprintf("m%d", i)})
	}
	got, _ := st.MessagesLatest(50)
	if len(got) != 50 || got[0].Body != "m10" || got[49].Body != "m59" {
		t.Errorf("Memory MessagesLatest: len=%d first=%q last=%q, want 50/m10/m59", len(got), got[0].Body, got[49].Body)
	}
}
